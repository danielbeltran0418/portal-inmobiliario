...
/**
 * Monta un par comprador/vendedor EFIMEROS (randomUUID en el correo, nunca
 * las cuentas fijas del seed -- ver el comentario de sesionVendedor() en
 * ayudantes.ts) mas dos propiedades del vendedor: una PUBLICADA de verdad
 * (con imagen y precio, que es lo que exigen los triggers
 * propiedades_exigir_imagen y propiedades_exigir_precio de la Task de SP3
 * antes de aceptar estado = 'publicada') y un BORRADOR que nunca se publica,
 * para el caso "propiedad no publicada".
 *
...
async function fixtura() {
  ...
  return {
    compradorCorreo, vendedorCorreo, password,
    compradorId, vendedorId, propiedadId, borradorId,
  }
}
